...
void ini_potential() {
  ...
  /* Interaction potential: |UXU'|*/
  for (i = 1; i <= M; ++i) {
    for (j = 1; j <= M; ++j) {
      UXU[i][j] = fabs(ucx[i] * usy[j] - usy[i] * ucx[j]);
    }
  }
}
...
void read_band(double ***ftu, double ***fmtu) {
  ...
  int shfty = 5;
  double ***tu, ***tmu;
  tu = f3tensor(1, M, 1, 10, 1, Ny);
  tmu = f3tensor(1, M, 1, 10, 1, Ny);
  PREP("Band", 0, "r")
    ...
    for (i = 1; i <= Nx; ++i) {
      for (j = 1; j <= Ny; ++j) {
        if (1 == k) {
          if (j >= 4 && j < 8) {
            ftu[k][i][j] =
                DENS * (PI * 3.0 / 2.0) / (2.0 * PI) * M * 4 / Ny +
                Nosi * (ran1(RSD) - 0.5);  //+0.5*exp(-pow(x2[j]-x2[N/2],2));
          } else {
            ftu[k][i][j] = 0;
          }
        } else {
