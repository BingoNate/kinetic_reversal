...
void output(int m, double ***ftu, double ***fmtu, double ***kru, double *x1,
  ...
  for (k = 1; k <= M; k++)
    for (i = 1; i <= Nx; i++) {
      for (j = 1; j <= Ny; j++) {
        fprintf_s(fp, "%d,%d,%e,%e\n", k, i, ftu[k][i][j], fmtu[k][i][j]);
      }
    }
  ...
  for (i = 1; i <= Nx; i++)
    for (j = 1; j <= Ny; j++)
      for (k = 1; k <= M; k++) {
        kkru[1][i][j] += ftu[k][i][j];
        kkru[10][i][j] += fmtu[k][i][j];
      }
  ...
  for (i = 1; i <= Nx; i++)
    for (j = 1; j <= Ny; j++)
      for (k = 1; k <= M; k++) {
        kkru[2][i][j] += ftu[k][i][j] * ucx[k];  // Polar.
        kkru[3][i][j] += ftu[k][i][j] * usy[k];
      }

  for (i = 1; i <= Nx; i++)
    for (j = 1; j <= Ny; j++) {
      kkru[2][i][j] /= kkru[1][i][j];
      kkru[3][i][j] /= kkru[1][i][j];
    }
  /*
  ...
  for (i = 1; i <= Nx; i++)
    for (j = 1; j <= Ny; j++)
      fprintf_s(fp, "%f,%f,%f,%f,%f,%f\n", x1[i], x2[j],
                x1[i] + kkru[2][i][j] * dxx, x2[j] + kkru[3][i][j] * dyy,
                kkru[2][i][j], kkru[3][i][j]);
...
*/
  ...
  for (i = 1; i <= Nx; i++)
    for (j = 1; j <= Ny; j++)
      for (k = 1; k <= M; k++) {
        kkru[2][i][j] += ftu[k][i][j] * (2 * ucx[k] * ucx[k] - 1);
        kkru[3][i][j] += ftu[k][i][j] * (2 * usy[k] * usy[k] - 1);
        kkru[4][i][j] += ftu[k][i][j] * 2 * usy[k] * ucx[k];
        kkru[7][i][j] += ftu[k][i][j] * ucx[k];
        kkru[8][i][j] += ftu[k][i][j] * usy[k];
      }
  ...
  for (i = 1; i <= Nx; i++)
    for (j = 1; j <= Ny; j++) {
      kkru[2][i][j] /= kkru[1][i][j];
      kkru[3][i][j] /= kkru[1][i][j];
      kkru[4][i][j] /= kkru[1][i][j];
      kkru[7][i][j] /= kkru[1][i][j];
      kkru[8][i][j] /= kkru[1][i][j];
    }
  ...
  /*
   for (i = 1; i <= Nx; i++) for (j = 1; j <= Ny; j++) {
     kkru[5][i][j] =
         sqrt(kkru[4][i][j] * kkru[4][i][j] -
              kkru[2][i][j] * kkru[3][i][j]);  // eigenvalue of order parameter
     kkru[6][i][j] = kkru[4][i][j] /
                    (kkru[5][i][j] - kkru[2][i][j]);  // angle of order
   parameter kkru[6][i][j] = atan(1 / kkru[6][i][j]);         // angle of order
   parameter
   }
   ...
   for (i = 1; i <= Nx; i++)
     for (j = 1; j <= Ny; j++)
       fprintf_s(fp, "%f,%f,%f,%f\n", x1[i], x2[j], kkru[6][i][j],
                 kkru[5][i][j]);  // x y angle magnitude
 ...
 */
  ...
  CLSP;
  free_f3tensor(kkru, 1, 10, 1, Nx, 1, Ny);
}
